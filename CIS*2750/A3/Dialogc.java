...
public class Dialogc extends JFrame implements KeyListener
{
	...
	private JMenuItem configChooseCompiler;
	...
	private ActionListener configChooseCompilerListener;
	 ...
	 */ 	
	public File theFileChooserBox(int flag)
	{
	...
	public void saveToTheFile()
	{
		try 
		{ 
			StringBuilder stringbuilder2 = new StringBuilder(documentPath);
			
			if(documentPath.endsWith(".config") == false)
			{
				stringbuilder2.append(".config");
			}		
			documentPath = stringbuilder2.toString();	
	...
	void clearOldConfigFile()
	{
		File configFile = new File(workingDirectoryFile.getPath()+"/"+documentName);
		File removeJavaFile = new File(configFile.getAbsolutePath().replace(".config",".java"));
		File removeClassFile = new File(configFile.getAbsolutePath().replace(".config",".class"));
		File removeClassFile1 = new File(configFile.getAbsolutePath().replace(".config","") + "$1" + ".class");
		File removeClassFile2 = new File(configFile.getAbsolutePath().replace(".config","") + "$2" + ".class");
		File removeClassFile3 = new File(configFile.getAbsolutePath().replace(".config","") + "$3" + ".class");
		File removeClassFile4 = new File(configFile.getAbsolutePath().replace(".config","") + "$4" + ".class");
		File removeException = new File(configFile.getAbsolutePath().replace(".config","") + "$IllegalFieldValueException" + ".class");
		
		DialogcCompiler clearOldFiles = null;
		try
		{
			if(removeJavaFile.exists()){clearOldFiles.run("rm "+removeJavaFile.getAbsolutePath() );}
			if(removeClassFile.exists()){clearOldFiles.run("rm "+removeClassFile.getAbsolutePath() );}
			if(removeClassFile1.exists()){clearOldFiles.run("rm "+removeClassFile1.getAbsolutePath() );}
			if(removeClassFile2.exists()){clearOldFiles.run("rm "+removeClassFile2.getAbsolutePath() );}
			if(removeClassFile3.exists()){clearOldFiles.run("rm "+removeClassFile3.getAbsolutePath() );}
			if(removeClassFile4.exists()){clearOldFiles.run("rm "+removeClassFile4.getAbsolutePath() );}
			if(removeException.exists()){clearOldFiles.run("rm "+removeException.getAbsolutePath() );}
	...
	public Dialogc()
	{
		...
		changesMadeFlag = false;
		statusBar = new JLabel("Current Project: "+documentName, JLabel.CENTER);
		statusBar.setVerticalTextPosition(JLabel.BOTTOM);
		statusBar.setHorizontalTextPosition(JLabel.CENTER);
		getContentPane().add(BorderLayout.SOUTH, statusBar);

		
		lexButton = new JRadioButton("External(Lex/Yacc)");
		pmButton = new JRadioButton("Internal(Parameter Manager)");
...
/*----------------------------------------------------------------------------------------------------------*/	
		...
		fileNewListener = new ActionListener()
		{
			...
			{							
				...
				//editorPane.setText(""); 
				setDocumentName("untitled");
				statusBar.setText(statusBar.getText()+"[modified]");
				statusBar.setText("Current Project: " + documentName);
				setTitle(getDocumentName());
				changesMadeFlag = false;
				try
				{
					File openMe = 	new File(documentName);
		...
		fileOpenListener = new ActionListener()
		{
			...
			{					
				...
				File openMe = theFileChooserBox(6);
			...
			public void actionPerformed(ActionEvent event)
			{
				...
				{					
					try
					{
						...
						clearOldConfigFile();
						if (br.readLine() != null && configFile.exists()) 
						{		
							String compileRunMe = javaCompilerFile.getName() + compilerOptionsString + " " + workingDirectoryFile.getPath()+"/"+ configFile.getName().replace(".config",".java");					
							DialogcCompiler theCompiler = new DialogcCompiler(configFile,workingDirectoryFile, mainDir);
							File javaFileExists = new File(configFile.getAbsolutePath().replace(".config",".java"));
							if(theCompiler.error_flag != 1)
							{
								theCompiler.run(compileRunMe);
							}
							else
							{
								JOptionPane.showMessageDialog(null, "Parse error detected while compiling","Error", JOptionPane.INFORMATION_MESSAGE);
							}
						}
					}
					catch (Exception e) 
				...
				if(lexButton.isSelected())
				{
					try
					{
						...
						{	
							...
							{
								theCompiler.run(compileRunMe);
								compileRunMe = JavaRunTimeFile.getName() +" -cp "+ RunTimeOptionsString + " " + workingDirectoryFile.getPath()+" "+documentName.replace(".config","");
								theCompiler.run(compileRunMe);
							}
							else
							{
...
/*----------------------------------------------------------------------------------------------------------*/			
		...
		configJavaCompilerListener = new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				...
				javaCompilerFile = theFileChooserBox(1);								
			}
		};	
		...
		configJavaRunTimeListener = new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				...
				JavaRunTimeFile = theFileChooserBox(1);					
			}
		};
		...
		configChooseCompilerListener = new ActionListener()
		{
			public void actionPerformed(ActionEvent event)
			{
				...
				panel.add(lexButton);			
				panel.add(pmButton);
				JOptionPane.showMessageDialog(null, panel, "Choose the Compiler", JOptionPane.PLAIN_MESSAGE);									
			}					
		};
...
/*----------------------------------------------------------------------------------------------------------*/		
		...
		fileSaveAs.setAccelerator(KeyStroke.getKeyStroke("ctrl S"));			
		fileQuit.setAccelerator(KeyStroke.getKeyStroke("ctrl Q"));	
		...
		compCompile.setAccelerator(KeyStroke.getKeyStroke("ctrl C"));
		compCompileRun.setAccelerator(KeyStroke.getKeyStroke("ctrl R"));				
		...
		configChooseCompiler = new JMenuItem("Compile Mode");		
		configJavaCompiler.setAccelerator(KeyStroke.getKeyStroke("ctrl P"));
		configCompilerOptions.setAccelerator(KeyStroke.getKeyStroke("ctrl I"));
		configJavaRunTime.setAccelerator(KeyStroke.getKeyStroke("ctrl Y"));
		configRunTimeOptions.setAccelerator(KeyStroke.getKeyStroke("ctrl T"));		
		configWorkingDirectory.setAccelerator(KeyStroke.getKeyStroke("ctrl W"));
		configChooseCompiler.setAccelerator(KeyStroke.getKeyStroke("ctrl K"));				
		...
		configChooseCompiler.addActionListener(configChooseCompilerListener);
		...
		configTab.add(configChooseCompiler);
		...
		helpHelp.setAccelerator(KeyStroke.getKeyStroke("ctrl H"));
		helpAbout.setAccelerator(KeyStroke.getKeyStroke("ctrl B"));		
...
/*-----------------------------------------------------------------------------------------------------*/
	...
	class MyDocumentListener implements DocumentListener 
	{
		public void insertUpdate(DocumentEvent e) 
		{
			...
			statusBar.setText("Current Project: " + documentName + " [modified]");
		}
